# -*- coding: utf-8 -*-
"""2021320322 윤민서 DATA410 Assignment1.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1PcmGX4HFI1PFztQ8vw9F3sQ-CEr7lcpu
...
df = pd.read_table('/content/ratings_train.txt', sep='\t', encoding='utf-8')
df.head()
...
for doc in documents:
    ...
    except: # 'nan'이 doc에 포함되어 있어 오류 발생
        pass

print(len(tokenized_documents))
print(tokenized_documents[:3])

model = Word2Vec(sentences=tokenized_documents, vector_size=100, window=5, min_count=5, sg=0)
...
plt.scatter(pca_result[:, 0], pca_result[:, 1], alpha=0.05)
for i, word in enumerate(words):
    if not i % 150:
        plt.annotate(word, xy=(pca_result[i, 0], pca_result[i, 1]), fontsize=6)
plt.title('PCA Visualization')
plt.show()

plt.figure(figsize=(10, 5), dpi=200)
plt.scatter(tsne_result[:, 0], tsne_result[:, 1], alpha=0.05)
for i, word in enumerate(words):
    if not i % 100:
        plt.annotate(word, xy=(tsne_result[i, 0], tsne_result[i, 1]), fontsize=6)
...
def remove_stopwords(sentences):
    ...
    for sentence in tqdm(sentences):
        cleaned_tokens_inner = []

        for token in sentence:
            token_str = str(token)

            if token_str not in stopwords:
                cleaned_tokens_inner.append(token_str)

        cleaned_tokens.append(cleaned_tokens_inner)

    return cleaned_tokens

cleaned_tokens = remove_stopwords(tokenized_documents)

print()
print(len(cleaned_tokens))
print(cleaned_tokens[:3])

model2 = Word2Vec(sentences=cleaned_tokens, vector_size=100, window=5, min_count=5, sg=0)
...
tsne = TSNE(n_components=2, perplexity=30, n_iter=300, verbose=True)
tsne_result = tsne.fit_transform(word_vectors)
...
"""# 추가 실험 2. (표제어, 명사) 추출 + 추가 실험 1"""

def process_tokens(sentences, lemmatize=True, nouns=True, stemming=True):
    processed_tokens = []

    for sentence in tqdm(sentences):
        processed_tokens_inner = []

        for token in sentence:
            token_str = str(token)
            pos_tags = mecab.pos(token_str)

            for pos_tag in pos_tags:
                word, tag = pos_tag
                if nouns and tag.startswith('N'):
                    processed_tokens_inner.append(word)
                if lemmatize and tag in ('Noun', 'Verb', 'Adjective', 'Adverb'):
                    processed_tokens_inner.append(mecab.lemmatize(word))
                # if stemming and tag in ('Noun', 'Verb'):
                #    processed_tokens.append(mecab.stem(word))

        processed_tokens.append(processed_tokens_inner)

    return processed_tokens

processed_tokens = process_tokens(cleaned_tokens)

print()
print(len(processed_tokens))
print(processed_tokens[:3])

model3 = Word2Vec(sentences=processed_tokens, vector_size=100, window=5, min_count=5, sg=0)
...
word_data = {word: (x, y) for word, (x, y) in zip(words, tsne_result)}

data = go.Scatter(
    x=[coord[0] for coord in word_data.values()],
    y=[coord[1] for coord in word_data.values()],
    mode='markers',
    text=list(word_data.keys()),
    hoverinfo='text'
)

fig = go.Figure(data=data)

fig.write_html('/content/tsne.html')